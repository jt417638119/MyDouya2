...
public class WriteArticleCommentActivity extends BaseActivity implements WriteView {
    private long id;
    ...
    private void init() {
        // 设置hint
        etWriteConent.setHint("写评论...");

        /*
        设置toolbar
         */
        getToolBar().setTitleVisible(false);
        getToolBar().setCustomTitle(R.string.lbl_write_comment);
        ...
        getToolBar().setToolbarRightIconOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                showTwoButtonDialog("确定发送此评论？","取消","确定",new NListner(),new PListner());
            }
        });
    }
    ...
    public void onError(String error) {
        Toast.makeText(getActivity(),"发送评论失败",Toast.LENGTH_SHORT).show();
        mDialog.dismiss();
        finish();
    }
    ...
    public void onSuccess() {
        Toast.makeText(getActivity(),"发送评论成功！",Toast.LENGTH_SHORT).show();
        mDialog.dismiss();
        finish();
    }
    ...
    public class PListner implements View.OnClickListener {
        ...
        @Override
        public void onClick(View v) {
            mWritePresenter.writeComment(etWriteConent.getText().toString(),id);
        }
    }
